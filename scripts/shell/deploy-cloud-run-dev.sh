#!/bin/bash
# Deploy ContextPilot Backend to Google Cloud Run (Development/Testing)
# This script allows deploying to a separate service for testing branches
...
echo -e "${BLUE}🚀 ContextPilot - Cloud Run Deployment (Development)${NC}"
echo "========================================"

# Allow custom service name via environment variable or argument
SERVICE_NAME=${1:-${CLOUD_RUN_SERVICE_NAME:-contextpilot-backend-dev}}
BRANCH_NAME=${BRANCH_NAME:-$(git branch --show-current 2>/dev/null || echo "unknown")}

echo -e "${YELLOW}⚠️  Deploying to DEVELOPMENT service: ${SERVICE_NAME}${NC}"
echo -e "${YELLOW}⚠️  Current branch: ${BRANCH_NAME}${NC}"
echo ""
...
echo ""
echo -e "${BLUE}4️⃣ Deploying to Cloud Run (DEV service)...${NC}"
...
gcloud run deploy $SERVICE_NAME \
    ...
    --min-instances 0 \
    --timeout 900 \
    --set-env-vars "ENVIRONMENT=development,STORAGE_MODE=cloud,REWARDS_MODE=firestore,EVENT_BUS_MODE=pubsub,USE_PUBSUB=true,GCP_PROJECT_ID=$PROJECT_ID,SANDBOX_ENABLED=true,CODESPACES_ENABLED=true,BRANCH_NAME=$BRANCH_NAME,GITHUB_ACTION_MODE=dev,GITHUB_BASE_BRANCH=$BRANCH_NAME" \
...
echo ""
echo -e "${GREEN}✅ ContextPilot Backend (DEV) is live!${NC}"
echo ""
echo -e "${BLUE}Service URL:${NC}"
echo "  $SERVICE_URL"
echo ""
echo -e "${YELLOW}⚠️  This is a DEVELOPMENT deployment${NC}"
echo -e "${YELLOW}⚠️  Production service: contextpilot-backend${NC}"
echo ""
echo -e "${BLUE}Test endpoints:${NC}"
echo "  Health: curl $SERVICE_URL/health"
echo "  Agents: curl $SERVICE_URL/agents/status"
echo "  Diagnostic: curl $SERVICE_URL/agents/development/diagnostic?workspace_id=contextpilot"
echo ""
...
echo "  1. Test the DEV service: $SERVICE_URL"
echo "  2. Update extension API URL temporarily to test: $SERVICE_URL"
echo "  3. When done testing, you can delete this service or keep it for future tests"
echo ""
