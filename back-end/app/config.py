...
class AppConfig:
    ...
    def _get_rewards_mode(self) -> RewardsMode:
        ...
        if mode_str not in ["firestore", "blockchain"]:
            logger.warning(f"Invalid REWARDS_MODE='{mode_str}'. Defaulting to 'firestore'.")
    ...
    def _log_config(self):
        ...
        logger.info(f"Gemini API:         {'✓ Configured' if self.gemini_api_key else '✗ Missing'}")
        logger.info(f"GitHub Token:       {'✓ Configured' if self.github_token else '✗ Missing'}")
        logger.info("=" * 60)
    ...
    def requires_gcp(self) -> bool:
        ...
        return (
            self.is_cloud_storage or
            self.event_bus_mode == EventBusMode.PUBSUB or
            self.is_blockchain_rewards  # Uses Firestore for pending tracking
        )
