...
class FirestoreRewardsAdapter(RewardsAdapter):
    ...
    async def track_action(
        ...
        logger.info(
            f"Tracked action: user={user_id}, action={action_type}, " f"points={points}"
        )
    ...
    async def get_balance(self, user_id: str) -> UserBalance:
        ...
        # Get recent actions (last 10) - simplified query to avoid index requirement
        actions_query = self.actions_col.where("user_id", "==", user_id).limit(10)
        actions_docs = await actions_query.get()

        # Sort in memory by timestamp (descending)
        actions_list = [doc.to_dict() for doc in actions_docs]
        actions_list.sort(key=lambda x: x.get("timestamp", ""), reverse=True)
        ...
        recent_actions = [
            RewardAction(
                ...
                blockchain_pending=action.get("blockchain_pending", False),
            )
    ...
    async def get_leaderboard(self, limit: int = 10) -> List[Dict]:
        ...
        query = self.balances_col.order_by(
    ...
    async def batch_mint(self, user_ids: List[str]) -> Dict[str, str]:
        ...
        """
        logger.info(
            f"Batch mint called in off-chain mode (no-op). " f"Users: {len(user_ids)}"
        )
        ...
        for user_id in user_ids:
            ...
            batch.update(
                balance_ref,
                {"pending_blockchain": 0, "last_mint": firestore.SERVER_TIMESTAMP},
            )
