...
class FirestoreService:
    ...
    def list_proposals(
        ...
        logger.info(f"[Firestore] Listed {len(proposals)} proposals (workspace: {workspace_id}, status: {status})")
        return proposals
    ...
    def save_agent_state(self, agent_id: str, workspace_id: str, state: Dict[str, Any]) -> str:
    ...
    def get_agent_state(self, agent_id: str, workspace_id: str) -> Optional[Dict[str, Any]]:
        """Get agent state"""
        state_id = f"{agent_id}_{workspace_id}"
        doc_ref = self.db.collection('agent_state').document(state_id)
        doc = doc_ref.get()
